...
Outgoing.prototype.placeOrder = function (id, contract, order) {
  ...
  args.push(order.orderMiscOptions);
  ...
  if (this._controller._serverVersion >= C.MIN_SERVER_VER.PEGGED_TO_BENCHMARK) {
    args.push(_nullifyMax(0));
    args.push(_nullifyMax(order.adjustedOrderType));
    args.push(_nullifyMax(order.triggerPrice));
    args.push(_nullifyMax(order.lmtPriceOffset));
    args.push(_nullifyMax(order.adjustedStopPrice));
    args.push(_nullifyMax(order.adjustedStopLimitPrice));
    args.push(_nullifyMax(order.adjustedTrailingAmount));
    args.push(_nullifyMax(order.adjustableTrailingUnit));
  }

  args.push(order.extOperator);
  args.push(order.softDollarTierName);
  args.push(order.softDollarTierValue);
  args.push(order.cashQty);
  args.push(order.mifid2DecisionMaker);
  args.push(order.mifid2DecisionAlgo);
  args.push(order.mifid2ExecutionTrader);
  args.push(order.mifid2ExecutionAlgo);
...
Outgoing.prototype.reqFundamentalData = function (reqId, contract, reportType) {
  ...
  args.push(contract.secType);
  args.push(contract.exchange);
  args.push(contract.primaryExchange);
  args.push(contract.currency);
  args.push(contract.localSymbol);
